...
export function computePurchaseLinePayloads(lines, landedCostExtraInput) {
  ...
  const raw = lines.map((row) => {
    ...
    return {
      ...
      warehouse: String(row.warehouse || 'الرئيسي').trim(),
      batch_number: String(row.batchNumber || '').trim(),
...
export function dbLineItemsToReceiveRows(lineItems) {
  ...
  return arr.map((item) => {
    ...
    return {
      ...
      warehouse: String(item.warehouse ?? 'الرئيسي'),
      batchNumber: String(item.batch_number ?? item.batchNumber ?? ''),
      sellPrice: null,
      stockFullPrice: null,
      brandGroup: '',

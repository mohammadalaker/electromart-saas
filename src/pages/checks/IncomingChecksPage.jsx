...
import { ArrowDownLeft, Loader2, Plus, Search, ChevronRight } from 'lucide-react';
...
export default function IncomingChecksPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [rows, cust] = await Promise.all([
        fetchChecksRegistry(store.id, { direction: 'incoming' }),
        fetchContactsByRole(store.id, 'customer'),
      ]);
      setChecks(rows);
      setContacts(cust);
    } catch (e) {
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6" dir="rtl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900">الشيكات الواردة</h1>
            <p className="text-sm text-slate-500">شيكات مستلمة من الزبائن</p>
          </div>
        </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            <label className="flex flex-col gap-1">
              <span className="text-xs font-bold text-slate-600">اختر العميل</span>
              <select
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
          <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 px-6 py-4">
            <h2 className="text-lg font-black text-slate-900">سجل الشيكات الواردة</h2>

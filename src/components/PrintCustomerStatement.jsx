...
/**
 * طباعة كشف حساب زبون — تخطيط A4، يتماشى مع كشف المورد.
 */
export default function PrintCustomerStatement({ data }) {
  if (!data) return null;
  ...
  return (
    <div
      className="print-invoice-root print-customer-statement max-w-[210mm] mx-auto bg-white text-slate-900 px-5 py-7 sm:px-9"
      dir="rtl"
    >
      <div className="relative overflow-hidden rounded-3xl border border-slate-200/90 shadow-sm mb-7 print:shadow-none print:border-slate-300">
        <div className="h-2 bg-gradient-to-l from-indigo-600 via-violet-600 to-indigo-800 print:h-1.5" />
        <header className="px-6 pt-6 pb-5 flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5 border-b border-slate-100/90">
          <div className="space-y-1">
            <p className="text-[10px] font-black tracking-[0.22em] text-indigo-800/85 uppercase">{brandNameEn}</p>
      ...
      <section className="mb-6 grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-3xl border border-indigo-100 bg-gradient-to-br from-indigo-50/90 via-violet-50/40 to-white p-6 shadow-sm print:border-indigo-200">
          <div className="flex items-center gap-2 mb-4">
            <span className="h-8 w-1 rounded-full bg-gradient-to-b from-indigo-500 to-violet-600" />
            <h2 className="font-title text-sm font-black text-indigo-950">بيانات الزبون</h2>
          </div>
          <p className="font-title text-xl font-black text-slate-900 leading-snug">{customerName || '—'}</p>
          {customerPhone ? (
        ...
        <div className="rounded-3xl border border-slate-200 bg-slate-50/70 p-5 flex flex-col justify-center">
          ...
          <dl className="space-y-2 text-sm">
            <div className="flex justify-between gap-2">
              <dt className="font-bold text-slate-600">إجمالي مدين (فواتير)</dt>
              <dd className="font-currency font-black text-emerald-800" dir="ltr" lang="en">
                ₪{totalDebit.toFixed(2)}
              </dd>
            </div>
            <div className="flex justify-between gap-2">
              <dt className="font-bold text-slate-600">إجمالي دائن (دفعات)</dt>
            ...
            <div className="flex justify-between gap-2 pt-1 border-t border-slate-200/80">
              <dt className="font-bold text-slate-800">الرصيد (مدين − دائن)</dt>
              <dd className="font-currency font-black text-indigo-900" dir="ltr" lang="en">
      ...
      <div className="relative overflow-hidden rounded-3xl border-2 border-indigo-200/90 bg-gradient-to-br from-indigo-50 via-violet-50/50 to-white px-6 py-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 print:border-indigo-300">
        <div className="absolute inset-0 bg-gradient-to-l from-indigo-500/5 to-transparent pointer-events-none print:hidden" />
        <span className="font-title relative text-lg font-black text-indigo-950">رصيد الذمة الختامي</span>
        <span
          className="font-title relative text-2xl sm:text-3xl font-black font-currency text-indigo-900 tracking-tight"
          dir="ltr"
          lang="en"
        >
      ...
      <p className="text-center text-[10px] text-slate-400 mt-8 pt-5 border-t border-slate-100 leading-relaxed">
        الرصيد = إجمالي المدين (فواتير) − إجمالي الدائن (دفعات) · {brandName} — {brandNameEn}
      </p>
    </div>
  );
}
